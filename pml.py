...
class PML:
    ...
    def RegressionCV(self, models_to_try, params_to_try, model_features, scoring_function='r2'):
        
        from sklearn.model_selection import GridSearchCV
        from sklearn.metrics import  roc_auc_score
       
        model_metrics = pd.DataFrame(columns=['params','quality','std'])
        model_metrics = []
        ...
        for i, current_model_name in enumerate(models_to_try.keys()):
            ...
            gridsearch = GridSearchCV(estimator = current_model, param_grid = param_map, scoring='r2', cv = 5, n_jobs=12, verbose=1)
            ...
            current_model_metrics = pd.DataFrame.from_records(list(zip(cv_results['params'], cv_results['mean_test_score'], cv_results['std_test_score'], cv_results['mean_fit_time'])), columns=['params','quality','std','fit_time'])
            current_model_metrics['model'] = current_model_name
            model_metrics.append(current_model_metrics)
            
            pd.concat(model_metrics).to_csv("model_metrics.csv", index=False)
            
        metrics = pd.concat(model_metrics)
    ...
    def ClassificationCV(self, models_to_try, params_to_try, model_features, scoring_function='roc_auc'):
        ...
        for i, current_model_name in enumerate(models_to_try.keys()):
            ...
            print("Processing model = {}".format(current_model))
            param_map = params_to_try[current_model_name]
            gridsearch = GridSearchCV(estimator = current_model, param_grid = param_map, scoring='roc_auc', cv = 5, n_jobs=12, verbose=1)
            X = model_features[current_model_name][0]
            y = model_features[current_model_name][1]
            gridsearch.fit(X, y)
    ...
    class FeatureImportance:
        ...
        def save_png(self, df_imp, filename):
            ...
            df_to_draw = result[['rank']].reset_index().sort_values("rank")[0:25]
